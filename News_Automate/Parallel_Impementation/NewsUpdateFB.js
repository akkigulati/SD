...
(async function () {
    try {
        ...
        await Fbupdate(tabs, news, FbCredential,browser)
...
async function fetchNews(tab, url, num) {
    ...
    await Promise.all([
        tab.click("div[data-tb-region=HomeTrending] .morediv a", { delay: 1000 }), tab.waitForNavigation({
...
async function Fbupdate(tab, news, FbCredential,browser) {
    ...
    for (let i = 0; i < news.length; i++) {
        ...
        let postPromise = fbpost(newTab,url,headline,img);
        pArray.push(postPromise);
    }
    await Promise.all(pArray);
}
async function fbpost(tab,url,headline,img){
        await tab.goto(url, { waituntil: "networkidle2" });
