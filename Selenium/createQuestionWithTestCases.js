...
(async function () {
    ...
    try {
        ...
        for (let i = 0; i < 1; i++) {
            ...
            await createTestCase(questions[i]);
        }
...
async function createTestCase(question) {
     await waitForLoader();
    // await driver.wait(swd.until.elementIsEnabled("li[data-tab=testcases]"));
    let testtab = await driver.findElement(swd.By.css("li[data-tab=testcases]"));
    await testtab.click();
    let testcases = question["Testcases"];
    for (let i = 0; i < testcases.length; i++) {
        let waitforit = Date.now() + 5000;
        //console.log(Date.now());
        while (Date.now() < waitforit) {
            // console.log("after some seconds");

        }
        //   await driver.wait(swd.until.elementsLocated(swd.By.css(".mlB.msT.testcase-alert-list")));
        // await driver.wait(swd.until.elementsLocated(swd.By.css(".btn.add-testcase.btn-green")));
        let addtest = await driver.findElement(swd.By.css(".btn.add-testcase.btn-green"));
        await addtest.click();

        let input = await driver.findElement(swd.By.css(".formgroup.horizontal.input-testcase-row.row .CodeMirror.cm-s-default.CodeMirror-wrap div textarea"));
        let output = await driver.findElement(swd.By.css(".formgroup.horizontal.output-testcase-row.row .CodeMirror.cm-s-default.CodeMirror-wrap div textarea"));


        await editorHandler(".formgroup.horizontal.input-testcase-row.row .CodeMirror.cm-s-default.CodeMirror-wrap div", input, testcases[i].Input);
        await editorHandler(".formgroup.horizontal.output-testcase-row.row .CodeMirror.cm-s-default.CodeMirror-wrap div", output, testcases[i].Output);
        let submit = await driver.findElement(swd.By.css(".btn.btn-primary.btn-large.save-testcase"));
        await submit.click();
        //waitForLoader()
