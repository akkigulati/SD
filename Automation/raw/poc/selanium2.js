...
return allmodulespromise;
  ...
  for(i=0;i<questions.length;i++){
   ...
   return questionWillbeClickedPromise
    ...
    let metadataFilePromise=fs.promises.readFile(metadataFile);
    return metadataFilePromise
      
  }).then(function(meta){
    //console.log(meta.length);
    meta = JSON.parse(meta);
    codePath=meta[0].path;
   // console.log(codePath);
    
    let codePromise=fs.promises.readFile(codePath,'utf8');
    return codePromise;
  }).then(function(code){
      console.log(code);
      let InputEditorWillbeSelectedPromise = driver.findElement(swd.By.css("#customInput")) //[] => this signify attribute.
      return Promise.all([InputEditorWillbeSelectedPromise,code]);
  }).then(function(codecopy){
  let p=  codecopy[0].sendKeys(codecopy[1]);
return p
})
.catch(function (err) {
